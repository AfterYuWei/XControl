...
/**
 * iOS 式底部操作表：spring 入场 + 全表可拖拽下拉关闭。
 * 拖拽 1:1 跟踪，松手按动量投射决定关闭或回弹（速度交接），
 * 背景遮罩随位移比例变淡（连续反馈）。
 */
export function MobileSheet({ open, onOpenChange, title, items }: MobileSheetProps) {
      ...
      <AnimatePresence>
        {open && (
          <DialogPrimitive.Portal forceMount>
            ...
            <DialogPrimitive.Content forceMount asChild aria-label={typeof title === 'string' ? title : '操作'}>
              <motion.div
                ref={contentRef}
                className="m-sheet"
                style={{ y }}

...
Production desktop runtime (tauri branch; replaced `electron/`). Rust shell: free loopback port + webview origin + spawn `xcontrol-server` sidecar (externalBin, env-driven, stdio to userData/logs) + health poll + graceful shutdown (Linux-only PR_SET_PDEATHSIG backstop). Data dir reuses the Electron-era path for seamless migration. Auth: REST Bearer header; WS `?access_token=`. In-app updates use independent stable/test channels via the rolling `tauri-update-channel` Release; test builds expose separate frontend/backend debug logs. Windows NSIS / macOS / AppImage support updating; deb/rpm manual. Commands: `npm run desktop:dev|desktop:build|desktop:smoke`. Docs: docs/DESKTOP_RUNTIME.md, docs/TAURI_MIGRATION.md.

Internal drag-and-drop is pointer-event based (Tauri dragDropEnabled intercepts HTML5 DnD): see web/src/hooks/usePointerDrag.ts + web/src/lib/dragRegistry.ts. External drag-in via Tauri onDragDropEvent; drag-out via Rust sftp_drag_out + crabnebula tauri-plugin-drag.

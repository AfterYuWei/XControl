...
XControl 桌面版是单进程 Tauri 2 应用。React 静态资源由 Tauri 打包，SQLite、加密、
SSH、SFTP、备份和同步均在 Rust 主进程内执行。

## 启动

1. Tauri 初始化单实例、深链、外链、对话框、拖拽和更新插件。
2. 从历史兼容的 `XControl` 用户目录打开数据库与密钥。
...
`XCONTROL_TAURI_SMOKE_OK` 后退出。CI 另执行 Rust clippy/test 和前端 lint/test/build。

...
4. **Windows updater 退出不会触发应用 RunEvent**：updater 启动 NSIS 前调用
   `AppHandle::cleanup_before_exit()` 后直接 `process::exit(0)`，原有
   `RunEvent::ExitRequested`/栈上 `ExitGuard` 无法关闭 sidecar。实现中把
   `BackendCleanupResource` 注册到 Tauri resource table，确保 updater cleanup
   时先优雅关闭/超时强杀 sidecar；NSIS `PREINSTALL/PREUNINSTALL` hook 再以
   `taskkill` + 删除旧文件兜底手动覆盖安装及旧版本升级。
